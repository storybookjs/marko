...
export default function start(render, { decorateStory } = {}) {
  ...
  const renderMain = forceRender => {
    ...
    const { storyId, docs } = storyStore.getSelection();

    const selection = storyStore.fromId(storyId);

    const { kind, name, getDecorated, id, parameters } = selection || {};
    ...
    if (docs && parameters && parameters.docs) {
      const StoryDocs = parameters.docs;
  ...
  if (isBrowser) {
    ...
    channel.on(Events.SET_CURRENT_STORY, ({ storyId: inputStoryId, name, kind }) => {
      ...
      storyStore.setSelection({ storyId, docs: false });
      setPath({ storyId, docs: false });
    });
  ...
  storyStore.on(Events.STORY_INIT, () => {
    const { storyId, docs } = initializePath();
    storyStore.setSelection({ storyId, docs });
  });
  ...
  const load = (req, m) => {
    ...
    configApi.configure(loadStories(req), m);
  };
