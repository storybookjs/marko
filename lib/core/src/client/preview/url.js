...
export const setPath = ({ storyId, docs = false }) => {
  const { path, selectedKind, selectedStory, ...rest } = qs.parse(document.location.search, {
    ignoreQueryPrefix: true,
  });
  const newPath = `${document.location.pathname}?${qs.stringify({ ...rest, id: storyId, docs })}`;
...
export const initializePath = () => {
  ...
  let { id: storyId, docs } = query; // eslint-disable-line prefer-const
  if (!storyId) {
    storyId = getIdFromLegacyQuery(query);
    if (storyId) {
      setPath({ storyId, docs });
    }
  }
  return { storyId, docs: docs === 'true' };
};
