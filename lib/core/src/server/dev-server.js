...
const usePrebuiltManager = async ({ configDir }) => {
  const prebuiltDir = path.join(__dirname, '../../prebuilt');
  const prebuiltIndex = path.join(prebuiltDir, 'index.html');

  const hasPrebuiltManager = await pathExists(prebuiltIndex);
  ...
  router.use('/', express.static(prebuiltDir, { index: false }));
  router.get('/', (request, response) => {
    response.set('Content-Type', 'text/html');
    response.sendFile(prebuiltIndex);
  });
  return true;
};
...
export default async function (options) {
  ...
  const prebuiltManager = await usePrebuiltManager({ configDir });

  const managerPromise = prebuiltManager
    ? Promise.resolve({})
    : loadManagerConfig({
        configType,
        outputDir,
