...
import loadManagerConfig, { getAutoRefs } from './manager/manager-config';
...
const canUsePrebuiltManager = async ({ prebuiltDir, configDir, options }) => {
  if (!options.managerCache) return false;
  ...
  const defaultAddons = ['@storybook/addon-links', '@storybook/addon-essentials'];
  // Addons we can safely ignore because they don't affect the manager
  const ignoredAddons = ['@storybook/preset-create-react-app', ...defaultAddons];
  ...
  if (defaultAddons.some((addon) => !addons.includes(addon))) return false;
  if (addons.some((addon) => !ignoredAddons.includes(addon))) return false;
...
export async function storybookDevServer(options) {
  ...
  const usePrebuilt = await canUsePrebuiltManager({ prebuiltDir, configDir, options });

  // Manager static files
  router.use('/', express.static(usePrebuilt ? prebuiltDir : outputDir));
  ...
  const [previewResult, managerResult] = await Promise.all([
    ...
    startManager({ startTime, options, configType, outputDir, configDir, usePrebuilt })
      .then((result) => {
