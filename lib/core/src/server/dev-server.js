...
export async function storybookDevServer(options) {
  ...
  const startServer = async (result) => {
    router.get('/', (request, response) => {
      response.set('Content-Type', 'text/html');
  ...
  const [managerResult, previewResult] = await Promise.all([
    startManager({ managerConfig, startTime }).then(startServer).catch(bailPreview),
    startPreview({ configType, outputDir, options, startTime }),
  ]);

  return { ...managerResult, ...previewResult, address, networkAddress };
}
