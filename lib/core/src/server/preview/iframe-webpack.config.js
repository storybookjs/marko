...
export default async ({
  ...
  entries.forEach((entryFilename) => {
    ...
    if (match) {
      ...
      // This code is NOT transpiled, must be ES5!
      virtualModuleMapping[entryFilename] = `
        ...
        var m = require(${JSON.stringify(configFilename)});

        if (m.args || m.argTypes) {
          logger.warn('Invalid args/argTypes in config, ignoring.', JSON.stringify(m));
        }
        if (m.decorators) {
          m.decorators.forEach(function(decorator){ return addDecorator(decorator); });
        }
        if (m.parameters || m.globalArgs || m.globalArgTypes) {
          addParameters(Object.assign({}, m.parameters, { globalArgs: m.globalArgs, globalArgTypes: m.globalArgTypes }));
        }
        if (m.parameterEnhancers) {
          m.parameterEnhancers.forEach(function(enhancer){ return addParameterEnhancer(enhancer)});
        }
      `;
    }
  });
