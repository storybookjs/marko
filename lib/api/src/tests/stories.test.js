...
import { STORY_ARGS_CHANGED, CHANGE_STORY_ARGS } from '@storybook/core-events';
...
describe('stories API', () => {
  ...
  describe('args handling', () => {
    it('changes args properly, per story when receiving STORY_ARGS_CHANGED', () => {
      const navigate = jest.fn();
      const store = createMockStore();

      const {
        api: { setStories, storyArgsChanged },
        init,
      } = initStories({ store, navigate });
      ...
      api.emit(STORY_ARGS_CHANGED, 'a--1', { foo: 'bar' });
    ...
    it('changeStoryArgs emits CHANGE_STORY_ARGS and does not change anything', () => {
      const navigate = jest.fn();
      const store = createMockStore();

      const {
        api: { setStories, changeStoryArgs },
        init,
      } = initStories({ store, navigate });
      ...
      const emit = jest.fn();
      init({ emit, on: jest.fn() });

      changeStoryArgs('a--1', { foo: 'bar' });
