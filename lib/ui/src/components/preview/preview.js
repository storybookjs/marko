...
class Preview extends Component {
  ...
  render() {
    ...
    if (previewTabs || parametersTabs) {
      ...
      const arrTabs = Object.keys(tabs).map(key => ({
      ...
      panels = panels
        .filter(panel => {
          ...
          return t === undefined || !t.hidden;
        })
        .map((panel, index) => ({ ...panel, index }))
        .sort((p1, p2) => {
          let index_1 = arrTabs.findIndex(tab => tab.id === p1.id);
          if (index_1 < 0) {
            index_1 = arrTabs.length + p1.index;
          }
          let index_2 = arrTabs.findIndex(tab => tab.id === p2.id);
          if (index_2 < 0) {
            index_2 = arrTabs.length + p2.index;
          }
        ...
        .map(panel => {
          const t = arrTabs.find(tab => tab.id === panel.id);
          if (t) {
            return { ...panel, title: t.title || panel.title, disabled: t.disabled };
          }
          return panel;
        });
    }
