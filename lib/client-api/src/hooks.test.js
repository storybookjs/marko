...
describe('Preview hooks', () => {
  ...
  describe('useArgs', () => {
    ...
    it('will emit CHANGE_GLOBAL_ARGS when called', () => {
      run(
        () => {},
        [
          storyFn => {
            useGlobalArgs()[1]({ a: 'b' });
            expect(mockChannel.emit).toHaveBeenCalledWith(CHANGE_GLOBAL_ARGS, { a: 'b' });
            return storyFn();
          },
        ],
        { globalArgs: {} }
  ...
  describe('useGlobalArgs', () => {
    ...
    it('will emit CHANGE_STORY_ARGS when called', () => {
      run(
        () => {},
        [
          storyFn => {
            useArgs()[1]({ a: 'b' });
            expect(mockChannel.emit).toHaveBeenCalledWith(CHANGE_STORY_ARGS, '1', { a: 'b' });
            return storyFn();
          },
        ],
        { id: '1', args: {} }
